...
CrossInk only credits active reading while pages are actually turning, so leaving a book open does not train your Pokémon. Each credited minute gives your lead Pokémon one experience point.

The game checks for a wild encounter every 15 minutes of credited reading. Encounters and item finds appear as a **!** on the dashboard, and up to three events can wait for you in the Pokémon menu. What appears is random, with the Pokémon and its level influenced by your progress through the current book.
...
4. Catch or pass on wild Pokémon. Your Party holds six; any additional Pokémon are sent to the PC Box.
5. Reorder your Party, deposit and withdraw Pokémon, and sort the PC Box by Pokédex number, name, or catch date.
...
This is a small reading companion, not a recreation of a full Pokémon game. It does not include battles, moves, breeding, trading, or combat statistics.
...
3. Back up `/.crosspoint/pokemon-v2-a.bin` and `/.crosspoint/pokemon-v2-b.bin` if they already exist.
4. Copy the extracted `update.bin` and `.crosspoint` folder to the SD-card root.
5. Merge `.crosspoint`; do not replace or delete the existing folder.
