...
## Task 1: Build and measure the integrated candidate

- Add the private `pokemon-x3` environment and compile guards: `CROSSINK_ENABLE_POKEMON`, `CROSSINK_POKEMON_DEFAULT_ENABLED`, and `CROSSINK_POKEMON_BETA_TOOLS`.
...
- Instrument activity entry/exit free heap, largest block, and touched task stack high-water marks in beta builds.
- Open/close the activity twenty times in simulation; reject heap drift above 2 KB.
- Commit: `chore: add pokemon v2 private build profile`.

## Task 2: Isolate the existing reader-menu crash with clean firmware
